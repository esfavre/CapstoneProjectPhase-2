 ...
 */
@WebServlet(urlPatterns = {"/AddBatch", "/UpdateBatch", "/DeleteBatch?id="})
public class BatchServlet extends HttpServlet {
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result > 0) {
			message = batch.getBatch_Group() + " " + batch.getBatch_Name() + " batch on " +  batch.getBatch_Date_Time().format(dateFormat) + " at " + batch.getBatch_Date_Time().format(timeFormat) + " was successfully updated in the database.";
		} else {
			message = batch.getBatch_Group() + " " + batch.getBatch_Name() + " batch at " +  batch.getBatch_Date_Time().format(dateFormat) + " at " + batch.getBatch_Date_Time().format(timeFormat) + " was not updated in the database. Please try again";
		}
